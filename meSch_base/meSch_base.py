...
class meSchBaseNode(Node):
    ...
    def __init__(self):
        ...
        self.quad_objects = [
            QuadObj("px4_100", 0, "Grounded", 120.0, False, 0, px4_100_int_soc, 0.0, False),
            QuadObj("px4_101", 0, "Grounded", 120.0, False, 0, px4_101_int_soc, 0.0, False),
            QuadObj("px4_102", 0, "Grounded", 120.0, False, 0, px4_102_int_soc, 0.0, False)
        ]
    ...
    def mission_status_cb(self, mission_status_msg):
        ...
        if (mission_status_msg.mission_start == False) and (mission_status_msg.mission_quit == False):
            ## For test
            if self.quad_objects[0].precomp_done:
                self.get_logger().info("px4_100 compilation done")
